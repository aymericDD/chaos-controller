...
func New(logger *zap.SugaredLogger, osArgs []string) (config, error) {
	...
	if cfg.Controller.DefaultDuration > 0 && cfg.Controller.DefaultDuration > cfg.Controller.MaxDuration {
		return cfg, fmt.Errorf("defaultDuration must be less than or equal to maxDefaultValue")
	}
